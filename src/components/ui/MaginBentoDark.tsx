...
const MagicBentoDark: React.FC<BentoProps> = ({
  ...
  return (
    <>
      ...
      <div 
        ref={gridRef}
        className="bento-section grid grid-cols-1 gap-4 md:grid-cols-3 md:grid-rows-2 h-full min-h-[600px] w-full max-w-6xl mx-auto p-4 relative"
      >
          {cardData.map((card, index) => {
            let gridClasses = "";
            if (index === 0) gridClasses = "md:col-span-2 md:row-span-1";
            else if (index === 1) gridClasses = "md:col-span-1 md:row-span-2";
            else if (index === 2) gridClasses = "md:col-span-1 md:row-span-1";
            else if (index === 3) gridClasses = "md:col-span-1 md:row-span-1";

            const baseClassName = `card flex flex-col justify-between relative min-h-[200px] w-full h-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${
